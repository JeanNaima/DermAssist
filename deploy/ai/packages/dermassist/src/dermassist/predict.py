...
from dermassist.vit_model import create_model
from dermassist.config import config
import cv2
import numpy as np
...
class SkinLesionPredictor:
    def __init__(self, model_path):
        ...
        self.idx_to_class = {int(v): k for k, v in self.config.class_names.items()}
        for i in range(self.config.num_classes):
            self.idx_to_class.setdefault(i, f"unknown_{i}")
    ...
    def predict(self, image_path):
        ...
        try:
            ...
            predicted_class = self.idx_to_class.get(
                predicted_index, f"unknown_{predicted_index}"
            )
            ...
            class_probs = {
                self.idx_to_class[i]: float(all_probs[i]) for i in range(len(all_probs))
            }
...
_PREDICTOR: Optional[SkinLesionPredictor] = None


def load(model_path: Optional[str] = None) -> SkinLesionPredictor:
    """Load once + cache."""
    global _PREDICTOR
    ...
    if model_path is None:
        model_path = os.getenv("DERMASSIST_WEIGHTS")

    if not model_path:
        model_path = os.path.join(
            "artifacts", "private", "dermassist", "saved_models", "best_model4.5.pth"
        )

    _PREDICTOR = SkinLesionPredictor(model_path)
    return _PREDICTOR


def predict_image(image_path: str, model_path: Optional[str] = None) -> Dict[str, Any]:
    predictor = load(model_path=model_path)
    out = predictor.predict(image_path)

    if isinstance(out, tuple) and len(out) == 4:
        label, confidence, probs, predicted_index = out
    elif isinstance(out, tuple) and len(out) == 3:
        label, confidence, probs = out
        predicted_index = -1
    elif isinstance(out, tuple) and len(out) == 2:
        label, confidence = out
        probs, predicted_index = {}, -1
    else:
        raise RuntimeError(f"Unexpected predictor.predict() return: {out}")

    return {
        "label": label,
        "confidence": float(confidence),
        "attributes": probs if isinstance(probs, dict) else {},
